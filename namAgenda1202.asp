...
if (session("Banco")="clinic5760" or session("Banco")="clinic6118" or session("Banco")="clinic105") and instr(ref("Locais"), "UNIDADE_ID")>0 then
...
<table class="table table-striped table-condensed table-hover" width="100%"><thead><tr><th colspan="3" style="min-width:200px" class="text-center pn">
    ...
    while not Horarios.EOF
        ...
        if MostraGrade then
        cProf = cProf+1
        %><tr><td colspan="3" nowrap class="nomeProf" style="background-color:<%= Cor %>"><%=left(ucase(Horarios("NomeLocal")&" "), 20)%><%=getNomeLocalUnidade(Horarios("UnidadeID"))%></td></tr><%
    if Horarios("TipoGrade")=0 then
        ...
        while Hora<=HoraA
            ...
            %>
            <tr class="p vazio l<%= LocalID %>" data-hora="<%=formatdatetime(Hora, 4)%>" data-pro="<%=ProfissionalID%>" id="<%=HoraID%>">
                <td width="1%"></td>
            ...
            elseif session("RemSol")<>"" then
                if Bloqueia="" then
                %>
                <tr class="p<%=ProfissionalID%> l<%= LocalID %> vazio" data-hora="<%=formatdatetime(Hora, 4)%>" data-pro="<%=ProfissionalID%>" data-id="<%=HoraID%>" id="<%=ProfissionalID&"_"&HoraID%>">
                    <td width="1%"></td>
                    <td width="1%"><button type="button" class="btn btn-xs btn-info"><%= formatdatetime(Hora,4) %></button></td>
                    <td colspan="4">
            ...
            %><tr onclick="abreAgenda('<%=HoraID%>', 0, '<%=Data%>', <%=LocalID%>, <%=ProfissionalID %>, '','<%=GradeID%>' )" class="p<%=ProfissionalID%> l<%= LocalID %>" data-pro="<%=ProfissionalID%>" data-id="<%=HoraID%>" data-hora="<%= ft(Hora) %>" id="<%=ProfissionalID&"_"&HoraID%>"><td width="1%" style="background-color:<%= Cor %>"></td><td width="1%"><button type="button" class="btn btn-xs btn-info"><%= ft(Hora) %></button></td><td><%= Tipo %></td></tr><%
			end if
...
<%
set comps=db.execute("select a.id, a.Data, a.Hora, a.LocalID, a.ProfissionalID, a.StaID, a.FormaPagto, a.Encaixe, a.Tempo, a.Procedimentos, p.NomePaciente, p.Nascimento, pro.NomeProfissional, pro.Cor, proc.NomeProcedimento, proc.Cor CorProcedimento from agendamentos a "&_
