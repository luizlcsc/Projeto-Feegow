<tr id="row<%=id%>"<%if id<0 then%> data-val="<%=id*(-1)%>"<%end if%>>
    ...
    <td><%=quickField("currency", "ValorUnitario"&id, "", 4, fn(ValorUnitario), " text-right disable", "", " required onkeyup=""recalc($(this).attr('id'))"" onchange='recorrenteLista();' ")%></td>
    <td><%=quickField("currency", "Desconto"&id, "", 4, fn(Desconto), " text-right disable", "", " required onkeyup=""recalc($(this).attr('id'))"" onchange='recorrenteLista();' ")%></td>
    <td><%=quickField("currency", "Acrescimo"&id, "", 4, fn(Acrescimo), " text-right disable", "", " required onkeyup=""recalc($(this).attr('id'))"" onchange='recorrenteLista();' ")%></td>
