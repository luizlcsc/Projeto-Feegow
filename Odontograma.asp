...
<script type="text/javascript">
    ...
    $.get('/feegow_components/odontograma/odontogramaAtendimento?P=<%=req("I")%>&U=<%=session("User")%>',
    function (data) {
...
//        I = id da invoice/atendimeto/sla
        ...
        $.get('feegow_components/odontograma?P=<%=req("I")%>&B=2898099&O=Invoice&U=<%=session("User")%>&I=<%=InvoiceID%>&load_all=' + first,
            function (data) {
                setTimeout(function () {
