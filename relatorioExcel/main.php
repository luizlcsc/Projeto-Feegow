...
//        VERIFICACAO DO RELATORIO
        $verify = "<td data-verify='0090'>";
        $arrayWithContent = array();
        for ($i = 0; $i < count($row); $i++) {
            $count = substr_count($row[$i], "td data-verify='0090'");
//            PRIMEIRA TR SÃO AS COLUNAS
            if ($i == 0) {
                $cols = explode("</td>", $row[$i]);
                foreach ($cols as $col) {
...
//                    TITULO E NOVA SHEET
                    $SheetID++;
                    $TRi = 0;
                    $value = str_replace(array($verify,"<td data-verify='0090' data-title='1'>", "</td>"), array("","",""), $row[$i]);
