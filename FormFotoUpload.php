...
error_reporting( E_ALL ); 
 ...
 $file = $_FILES[$_GET['Col']];
 if(!preg_match('/^image\//' , $file['type']) 
	|| !preg_match('/\.(jpe?g|gif|png)$/' , $file['name'])
		|| getimagesize($file['tmp_name']) === FALSE
	) {
	$result['status'] = 'ERR';
	$result['message'] = 'Formato de arquivo inválido.';
 }
 else if($file['size'] > 1100000) {
	$result['status'] = 'ERR';
	$result['message'] = 'Por favor, selecione um arquivo menor.';
 }
 else if($file['error'] != 0 || !is_uploaded_file($file['tmp_name'])) {
	$result['status'] = 'ERR';
	$result['message'] = 'Erro não especificado. Por favor, contate a Feegow Technologies.';
 }
 else {
	...
	$result['status'] = 'OK';
	$result['message'] = 'Foto alterada com sucesso!';
	$result['url'] = '';
...
header("location:FormFotoUploadSave.asp?FTipo=".$_GET['FTipo']."&PacienteID=".$_GET['PacienteID']."&FormID=".$_GET['FormID']."&ModeloID=".$_GET['ModeloID']."&Col=".$_GET['Col']."&Action=Insert&FileName=".$fileName);
?>