...
response.Buffer
%>
<h3 class="text-center">Produ&ccedil;&atilde;o - Sintético</h3>
...
'    db_execute("delete from tempfaturamento where sysUser="&session("User"))
	...
	<div>
        ...
        <table id="t<%=nomeTabela %>" class="table table-bordered table-hover hidden" width="100%">
	        <thead>
                <tr class="success">
        	        <th>PRONT</th>
        ...
        '			quartoProcedimento = G("quartoProcedimento")
                    ...
                    if Oculta = 0 then
				        ...
				        <tr class="linhaPac">
					        ...
					        <td><%=left(G("NomeProcedimento")&" ", 25)%></td>
                            <td><%=Situacao %></td>
					        <td><%=Convenio%>
                                <%'="<br />"&strFat %>
					        </td>
            ...
            <tfoot>
                ...
                <%
                if req("Agrupado")="Procedimento" then
                %>
                ...
                <table class="table table-striped table-bordered table-hover" width="100%">
                  ...
                  <tbody>
	                <%
	                set res = db.execute("select distinct tf.ConvenioID, if(tf.ConvenioID=0, 'Particular', conv.NomeConvenio) NomeConvenio, (select count(*) from tempfaturamento where ConvenioID=tf.ConvenioID and sysUser="&session("User")&") qtd, (select sum(valor) from tempfaturamento where sysUser="&session("User")&" and ConvenioID=tf.ConvenioID) total from tempfaturamento tf left join convenios conv on conv.id=tf.ConvenioID where tf.sysUser="&session("User")&" order by conv.NomeConvenio")
	                TotalProcs = 0
	                TotalProcsValor = 0
	                while not res.EOF
		                TotalProcs = TotalProcs+ccur(res("qtd"))
		                TotalProcsValor = TotalProcsValor+ccur(res("total"))
                        NomeConvenio = res("NomeConvenio")
                        if isnull(NomeConvenio) then
                            NomeConvenio = "-"
                        end if
		                %>
		                <tr>
