...
if caixa.eof then
    ...
    <div class="modal-body">
      ...
      <table class="table table-striped table-hover table-bordered">
        ...
        <tbody>
			...
			"(select sum(Value) from sys_financialmovement where (Type='Pay' or Type='Transfer') and AccountAssociationIDDebit=7 and AccountIDDebit="&caixa("id")&" and PaymentMethodID=m.PaymentMethodID) as Recebimentos, "&_ 
			"(select sum(Value) from sys_financialmovement where (Type='Pay' or Type='Transfer') and AccountAssociationIDCredit=7 and AccountIDCredit="&caixa("id")&" and PaymentMethodID=m.PaymentMethodID) as Pagamentos, "&_ 
			" pm.PaymentMethod "&_ 
			...
			" UNION ALL "&_
			"select mOutros.PaymentMethodID, sum(mOutros.Value), 0, pmOutros.PaymentMethod from sys_financialmovement mOutros "&_ 
			...
			while not mov.eof
				...
				if isnull(Recebimentos) then Recebimentos=0 end if
				Pagamentos = mov("Pagamentos")
				if isnull(Pagamentos) then Pagamentos=0 end if
    ...
    <div class="modal-footer">
			...
			%>
		<% if session("Banco")="clinic5760" or session("Banco")="clinic105" then %>
