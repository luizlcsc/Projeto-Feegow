<li id="<%=CampoID%>" class="campo" data-row="<%=pTop%>" style="text-align:left" data-col="<%=pLeft%>" data-sizex="<%=Colunas%>" data-sizey="<%=Linhas%>"><span class="botoes"><% If TipoCampoID=13 Then 
	%><button onClick="menu(<%=CampoID%>)" class="btn btn-xs btn-warning" style="z-index:500; width:17px; margin-top:-20px;"><i class="far fa-plus"></i></button>&nbsp;<button onClick="editField(<%=CampoID%>)" class="btn btn-xs btn-primary" style="z-index:500; width:17px; margin-top:-20px;"><i class="far fa-edit"></i></button>&nbsp;<button onClick="removeCampo(<%=CampoID%>, 0)" class="btn btn-xs btn-danger" style="z-index:500; width:17px; margin-top:-20px;"><i class="far fa-remove"></i></button><%
else
	...
	%></span><%
	  select case TipoCampoID
	  	...
	  	case 9'tabela
			...
			%><th width="1%"><button type="button" onClick="addRow(<%=CampoID%>, 0, 'Add')" class="btn btn-xs btn-primary btn-20"><i class="far fa-plus"></i></button></th></thead><tbody><%
			...
			while not pMod.EOF
				...
				wend
				%><td><button type="button" class="btn btn-xs btn-danger btn-20" onClick="addRow(<%=pMod("id")%>, 0, 'Remove')"><i class="far fa-remove"></i></button></td></tr><%
			pMod.movenext
			wend
			pMod.close
			set pMod = nothing
