...
if request.QueryString("X")<>"" then
	db_execute("delete from tissprofissionaishonorarios where id="&request.QueryString("X"))
end if
%>
<table width="100%" class="table table-striped table-bordered table-condensed table-hover">
  <thead>
    <tr>
      ...
      <th width="30" align="center" nowrap><button type="button" class="btn btn-info btn-xs" onClick="itemHonorarios('Profissionais', <%=request.QueryString("I")%>, 0);"><i class="fa fa-plus"></i></button></th>
    </tr>
  </thead>
  <tbody>
  <%
  set p = db.execute("select * from tissprofissionaishonorarios where GuiaID="&request.QueryString("I"))
  while not p.eof
  ...
  %>
    <tr id="lProfissionais<%=p("id") %>">
      <td align="center"><button type="button" class="btn btn-xs btn-success" onClick="itemHonorarios('Profissionais', <%=request.QueryString("I")%>, <%=p("id")%>);"><i class="fa fa-edit"></i></button></td>
      ...
      <td align="center"><button type="button" class="btn btn-xs btn-danger" onClick="atualizaTabela('tissprofissionaishonorarios', 'tissprofissionaishonorarios.asp?I=<%=request.QueryString("I")%>&X=<%=p("id")%>')"><i class="fa fa-remove"></i></button></td>
    </tr>
    <tr>
