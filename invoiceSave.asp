...
if erro="" then
	...
	if existePagto="" then
        ...
        if scp()=1 then
			sqlInvoice = "update sys_financialinvoices set Voucher='"&Voucher&"', Rateado="&contaRateada&", AccountID="&AccountID&", AssociationAccountID="&AssociationAccountID&", Value="&treatvalzero(ref("Valor"))&", Tax=1, Currency='BRL', Recurrence="&treatvalnull(ref("Recurrence"))&", RecurrenceType='"&ref("RecurrenceType")&"', FormaID="&treatvalzero(splForma(0))&", ContaRectoID="&treatvalzero(splForma(1))&", TabelaID="& varTabelaID &", ProfissionalSolicitante='"&ref("ProfissionalSolicitante")&"', nroNFe='"& ref("nroNFe") &"', CompanyUnitID="&treatvalzero(ref("CompanyUnitID"))&", sysActive=1 "& sqlCaixaID & sqlUsuario & gravaData &" where id="&InvoiceID
...
'			end if
    ...
    if existePagto="" or aut("|tabelacontapagaA|")=1 then
        sqlAtualizaTabela=" TabelaID="& varTabelaID &", "
    end if
