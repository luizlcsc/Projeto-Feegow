	  ...
	  select case TipoCampoID
	  	...
	  	case 2'Data
			if LadoALado="S" then
				...
				set perm = db.execute("select Permissoes from buipermissoes where FormID="&ModeloID)
				disabled = ""
                if aut("prescricoesX")>0 or instr(perm("Permissoes"), "XP")>0 then
					disabled = "disabled"
				end if
...
$(function() {
	...
	if(location.protocol == 'file:') alert("For uploading to server, you should access this page using a webserver.");
	  	...
	  	case 8'textarea
            ...
            <div id="input_<%=CampoID%>mem" style="overflow:auto" <%if negadoX<>"S" then%> contenteditable="true"<% End If %> class="campoInput memorando postvalue form-control" <%if negadoX<>"S" then%> onblur="alt(); $('#input_<%=CampoID %>').html( $(this).html() )"<% End If %>  data-campoid="<%=CampoID%>" name="input_<%=CampoID%>mem" tabindex="<%=Ordem%>"><%=ValorPadrao%></div>
