...
if r="pacientes" then
    if session("Logo")="UNIMEDLONDRINA.png" then
        sql = "select id, NomePaciente, CPF from pacientes where NomePaciente like '"& typed &"%' and id not like '"& id &"' UNION ALL SELECT 1000000000+id, concat(NomePaciente, ' (Base Unimed)'), CPF FROM "&session("Banco")&".pacientes WHERE NomePaciente like '"& typed &"%'"
    else
