...
<%
if req("X")<>"" then
	db_execute("delete from tissprofissionaishonorarios where id="&req("X"))
end if
%>
<table width="100%" class="table table-striped table-bordered table-condensed table-hover">
  <thead>
    <tr>
      ...
      <th width="30" align="center" nowrap><button type="button" class="btn btn-info btn-xs" onClick="itemHonorarios('Profissionais', <%=req("I")%>, 0);"><i class="far fa-plus"></i></button></th>
    </tr>
  </thead>
  <tbody>
  <%
  set p = db.execute("select * from tissprofissionaishonorarios where GuiaID="&req("I"))
  while not p.eof
  ...
  %>
    <tr id="lProfissionais<%=p("id") %>">
      <td align="center"><button type="button" class="btn btn-xs btn-success" onClick="itemHonorarios('Profissionais', <%=req("I")%>, <%=p("id")%>);"><i class="far fa-edit"></i></button></td>
      ...
      <td align="center"><button type="button" class="btn btn-xs btn-danger" onClick="atualizaTabela('tissprofissionaishonorarios', 'tissprofissionaishonorarios.asp?I=<%=req("I")%>&X=<%=p("id")%>')"><i class="far fa-remove"></i></button></td>
    </tr>
    <tr>
