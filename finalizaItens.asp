...
if AtendimentoID="N" then
	set at = db.execute("select * from atendimentos where sysUser="&session("User")*(-1))
end if
Acao = ref("A")
Tipo = ref("T")
II = ref("II")
...
if Acao="" then
	...
	<table width="100%" class="duplo table table-striped table-bordered table-condensed">
		...
		Total = 0
		Subtotal = 0
		set itens=db.execute("select * from atendimentosprocedimentos where AtendimentoID="&AtendimentoID)'.............
		while not itens.eof
