...
if selectID="CodigoProcedimento" then
    ...
    ProcedimentoID=0
    sql = "select COUNT(pv.id)Qtd,pv.ProcedimentoID, p.NomeProcedimento from tissprocedimentostabela pt LEFT JOIN tissprocedimentosvalores pv on pv.ProcedimentoTabelaID=pt.id LEFT JOIN procedimentos p on p.id=pv.ProcedimentoID where pt.Codigo='"&codigo&"' And PV.ProcedimentoID is not null and pt.TabelaID="&treatvalnull(tabela)&" GROUP BY pv.ProcedimentoID ORDER BY Qtd DESC LIMIT 1"
