...
%>
				...
				if (lcase(tableName)="funcionarios" or lcase(tableName)="profissionais") and session("Status")<>"T" then
                    ...
                    btnInserir = "<button type=""button"" class=""btn btn-sm btn-success"" onclick=""conf()""><i class=""fa fa-plus""></i> INSERIR</button>"
                %>
                
                <script type="text/javascript">
					<%
					if session("Admin")=1 then
						set contaUsu = db.execute("select ((select count(*) from profissionais where sysActive=1 and Ativo='on')+(select count(*) from funcionarios where sysActive=1 and Ativo='on')) as total")
						set valorUsu = db.execute("select valorusuario from cliniccentral.licencas where id="&replace(session("banco"), "clinic", ""))
						ValorAtual = ccur(contaUsu("total"))*ccur(valorUsu("valorUsuario"))
						NovoValor = ValorAtual+ccur(valorUsu("valorUsuario"))
        ...
        <li<%if request.QueryString("Initial")="" then%> class="active" <%end if%>>
                                ...
                                while nchr<90
                                    ...
                                    <li<%if request.QueryString("Initial")=vchr then%> class="active"<%end if%>>
                                        <a href="./?P=<%=request.QueryString("P")%>&Pers=<%=request.QueryString("Pers")%>&Initial=<%=vchr%>"><%=vchr%></a>
                                    </li>
                                    <%
                                wend
                                %>
...
<%
                    ...
                    if req("sysActive")="" then
                        if lcase(tableName)="profissionais" or lcase(tableName)="funcionarios" then
    					    sqlReg = "select * from "&tableName&" where sysActive=1 "&sqlInitial & sqlConstatus &" order by ativo desc, "&initialOrder&" limit "&partirde&limite
                        elseif lcase(tableName)="procedimentos" then
    					    sqlReg = "select * from "&tableName&" where sysActive=1 "&sqlInitial & sqlConstatus &" order by ativo desc, "&initialOrder&" limit "&partirde&limite
                        else
    					    sqlReg = "select * from "&tableName&" where sysActive=1 "&sqlInitial & sqlConstatus &" order by "&initialOrder&" limit "&partirde&limite
                        end if
                    else
    					sqlReg = "select * from "&tableName&" where sysActive="&req("sysActive")&" "&sqlInitial& sqlConstatus & " order by "&initialOrder&" limit "&partirde&limite
                    end if
					...
					if lcase(tableName)="pacientes" then
						sqlReg = "select * from pacientes where trim(NomePaciente) like '%"&q&"%' or replace(replace(CPF,'.',''),'-','') like replace(replace('"&q&"%','.',''),'-','') or Tel1 like '%"&q&"%' or Tel2 like '%"&q&"%' or Cel1 like '%"&q&"%' or Cel2 like '%"&q&"%' or id = '"&q&"' "& sqlNasc &" and sysActive=1 order by NomePaciente"
				...
				if not aass.eof then
					if (lcase(tableName)="pacientes" and aut("contapacV")=1) or ( lcase(tableName)="profissionais" and aut("contaprofV")=1 ) or ( (lcase(tableName)="sys_financialcurrentaccounts" or lcase(tableName)="fornecedores" or lcase(tableName)="convenios" or lcase(tableName)="funcionarios") and aut("movementV")=1 ) then
						associacao = aass("id")
					end if
				end if
				'<--- Rotina pra mostrar ou não o link de extrato
				...
				<table id="table-<%=tableNameXXXX%>" class="table table-striped table-bordered table-hover">
					...
					<tbody>
						...
						<tr>
							...
							<%if lcase(tableName)="pacientes" then%>
                            <td class="hidden-xs"><%if session("banco")="clinic1612" or session("banco")="clinic3610" or session("banco")="clinic105" or session("banco")="clinic3859" or session("banco")="clinic5491" then%><%=reg("idImportado")%><%else%><%=reg("id")%><%end if%></td>
							<%
							end if
...
'										response.Write(sql)
									...
									elseif splfieldFieldType(i)=24 then
										if valor=0 then
											set emp = db.execute("select NomeEmpresa from empresa")
											if not emp.eof then
												valor = emp("NomeEmpresa")
											else
												valor = "-"
											end if
										else
											set fil = db.execute("select UnitName from sys_financialcompanyunits where id="&valor)
											if not fil.eof then
												valor = fil("UnitName")
											else
												valor = "-"
                                    ...
                                    if (lcase(tableName)="profissionais" or lcase(tableName)="funcionarios" or lcase(tableName)="procedimentos") and splArrInitialOrder(i)=1 then
                                        if reg("Ativo")="" then
							...
							if req("P")="buiforms" then
							set conta = db.execute("select count(*) total from buiformspreenchidos where ModeloID="&reg("id"))
            ...
            <%
				...
				if paginar=1 then
                    ...
                    <ul class="pagination pagination-sm">
					<%
					contaLetra = db.execute("select count(*) as totalletra from "&tableName&" where sysActive=1 "&sqlInitial)
					...
					while nPagina<paginas
                        ...
                        <li<%if (request.QueryString("Pag")="" and nPagina=1) or request.QueryString("Pag")=cstr(nPagina) then%> class="active"<%end if%>>
                            <a href="./?P=<%=request.QueryString("P")%>&Pers=<%=request.QueryString("Pers")%>&Initial=<%=request.QueryString("Initial")%>&Pag=<%=nPagina%>"><%=nPagina%></a>
                        </li>
						<%
						end if
					wend
