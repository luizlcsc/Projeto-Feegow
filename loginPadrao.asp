	...
	if erro<>"" then
        ...
        if not ConexaoAbertaSucesso then
            ...
            if erro="" then
                ...
                if session("UnidadeID")&"" <> "-1" and AppEnv="production" then
                    set UnidadeVersaoSQL = dbc.execute("SELECT Versao FROM cliniccentral.licenca_unidades_versao WHERE LicencaID='"& replace(licencas,"|","") &"' AND (TipoUsuario='any' or TipoUsuario='"&session("Table")&"') AND UnidadeID="&treatvalzero(session("UnidadeID")))
