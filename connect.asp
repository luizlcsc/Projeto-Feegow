...
'	set fs=nothing
    ...
    IF tipoLog = "update" AND recursoLog = "tarefas" THEN
    	gravaLog txt, "AUTO"
        db.execute(txt)
    ELSE
    	db.execute(txt)
    ...
    'if tipoLog = "update" or tipoLog = "delete" and (session("Banco")="clinic522" or session("Banco")="clinic6118") then
    '    gravaLog txt, "AUTO"
    '    db.execute(txt)
    'elseif tipoLog = "insert" and (session("Banco")="clinic522" or session("Banco")="clinic6118") then
       ' response.write(txt)
        'response.end
    '    db.execute(txt)
    '    gravaLog txt, "AUTO"
                ...
                if (tipoLog="insert") then
                    ...
                    if InStr(tabelas,"|"&recurso&"|") then
                        ...
                        next
                         ...
                         db.execute("insert into log (Operacao, I, recurso, colunas, valorAnterior, valorAtual, sysUser) values ('"&operacao&"', "&idLog&", '"&recurso&"', '"&colunas&"', '"&valorAnterior&"', '"&valorAtual&"', "&session("User")&")")
                         if(operacaoForce <> "AUTO") then
                            db.execute(query)
                         end if
                    end if
                    ...
                    if InStr(tabelas,"|"&recurso&"|") then
                        ...
                        while not record.eof
                            ...
                            if(colunas <> "|") then
                                ...
                                db.execute("insert into log (Operacao, I, recurso, colunas, valorAnterior, valorAtual, sysUser) values ('"&operacao&"', "&record("id")&", '"&recurso&"', '"&colunas&"', "&valoresAnteriores&", "&valoresAtuais&", "&session("User")&")")
                             end if
                            record.movenext
                        wend
                    end if
