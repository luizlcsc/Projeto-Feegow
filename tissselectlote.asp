...
<select name="LoteID" class="form-control select2-single">
    ...
    <option value="0"<%if request.QueryString("LoteID")="0" then%> selected<%end if%>>Guias Fora de Lote</option>
    <option value="-1"<%if request.QueryString("LoteID")="-1" then%> selected<%end if%>>Guias não pagas</option>
    <%
	if request.QueryString("ConvenioID")<>"" and request.QueryString("T")<>"" then
		...
		while not lotes.eof%>
        	<option value="<%=lotes("LoteID")%>"<%if cstr(lotes("LoteID"))=request.QueryString("LoteID") then%> selected<%end if%>>Lote <%=lotes("Lote")%></option>
			<%
		lotes.movenext
		wend
		lotes.close
