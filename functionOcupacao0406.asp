...
function ocupacao(De, Ate, refEspecialidade, reffiltroProcedimentoID, rfProfissionais, rfConvenio, rfLocais)
                ...
                while Data<=Ate
                    ...
                    while not comGrade.eof
                        ...
                        if not Horarios.eof then
							...
							'response.write( txtGradeOriginal )
                            ...
                            while not comps.EOF
                                ...
                                db.execute("INSERT INTO rel_ocupacao SET sysUser="& session("User") &", Data="& mydatenull(Data) &", Hora="& mytime(Horario) &", StaID="& StaID &", Situacao='A', ProfissionalID="& treatvalzero(ProfissionalID) &", EspecialidadeID="& replace(Especialidades, "|", "") &", LocalID="& treatvalzero(LocalID) &", UnidadeID="& treatvalzero(UnidadeID) &", Encaixe="& treatvalnull(comps("Encaixe")) &", GradeOriginal="& GradeOriginal)
    ...
    HoraDe = bloq("HoraDe")
    HoraA = bloq("HoraA")
        db.execute("UPDATE rel_ocupacao SET Situacao='B', GradeOriginal=NULL WHERE sysUser="& session("User") &" AND Data="& mydatenull(Data) &" AND ProfissionalID="& treatvalnull(ProfissionalID) &" AND Hora BETWEEN "& mytime(HoraDe) &" AND "& mytime(HoraA) &" AND Situacao IN('V', 'A')")
