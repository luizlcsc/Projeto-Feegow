...
if not Prof.EOF then
    escreveProfissional = "$(""#nome-profissional"").html("""&Prof("NomeProfissional")&""");"
end if
escreveData = "$(""#data-escrita"").html('"&formatdatetime(Data, 1)&"');"
...
'COMPROMISSOS N�O S�O agendamentos!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	elseif Classe="C" then
		if Remarcar="S" then
			...
			if totalAgendamentos=1 then
				contaagendamentos=1
				%><!--#include file="ConteudoAgendamento.asp"--><%
...
<%
response.Write(escreveProfissional)
response.Write(escreveData)
%>
...
<script type="text/javascript">
	...
	<%
	set pDiasNA=db.execute("select * from Horarios where ProfissionalID = "&ProfissionalID&" and Atende like ''")
	while not pDiasNA.eof
		%>
		$(".<%=weekdayname(pDiasNA("Dia"))%>").addClass("danger");
		<%
	pDiasNA.movenext
	wend
	pDiasNA.close
