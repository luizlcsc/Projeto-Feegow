...
response.Buffer
%>
<h3 class="text-center">Produ&ccedil;&atilde;o Externa</h3>
<h5 class="text-center">Per&iacute;odo - <%=DataDe%> at&eacute; <%=DataAte%></h5>

<%
if req("ProfissionalID")="" then
    erro = "Selecione ao menos um profissional externo"
end if
...
if erro="" then
    ...
    for i=0 to ubound(splU)
	      Total = 0
	      set prof = db.execute("select p.* from profissionalexterno p where p.sysActive=1 "&sqlProf&" and p.id in("&req("ProfissionalID")&") order by p.NomeProfissional")
	      while not prof.eof
		    if splU(i)="0" then
        ...
        <table id="t<%=nomeTabela %>" class="table table-bordered table-hover" width="100%">
	        <thead>
                <tr class="success">
                    ...
                    <th class="text-center hidden-print">FORMA DE PAGAMENTO</th>
        ...
        '			quartoProcedimento = G("quartoProcedimento")
	        ...
	        '			db_execute("insert into tempfaturamento (sysUser, ProcedimentoID, ConvenioID, Valor) values ("&session("User")&", "&ProcedimentoID&", "&treatvalzero(ConvenioID)&", "&treatvalzero(Valor)&")")
				        ...
				        <tr class="linhaPac">
					        ...
					        <td class="text-right hidden-print"><%=DetPag%></td>
