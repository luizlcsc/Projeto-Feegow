...
if session("Banco")="clinic5760" then
    'response.Write(selectUnion &"<br>")
		    ...
		    %>
		    <div type="button" class="select-insert-item text-left" style="font-size:10px; padding-left:4px; text-align:left; width:300px" data-title="<%=list("coluna")%>" data-valor="<%=list("associacao")&"_"&list("id")%>" <%= replace(replace(request.Form("othersToSelect"), "onchange", "onclick"), "this.id", "'select-"&ref("selectID")&"'") %>>
                <%=list("coluna")%></div>
		    <%
            end if
	list.movenext
