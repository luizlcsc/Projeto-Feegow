...
function ocupacao(De, Ate, refEspecialidade, reffiltroProcedimentoID, rfProfissionais, rfConvenio, rfLocais)
        ...
        rfEspecialidade = Especialidades
                ...
                while Data<=Ate
                    ...
                    while not comGrade.eof
                        ...
                        'if (session("Banco")="clinic5760" or session("Banco")="clinic6118" or session("Banco")="clinic5968" or session("Banco")="clinic105" or session("Banco")="clinic6259") and instr(rfLocais, "UNIDADE_ID")>0 then
                        ...
                        if not Horarios.eof then
                                ...
                                if MostraGrade then
                                    ...
                                    if Horarios("TipoGrade")=0 then
                                        ...
                                        while Hora<=HoraA
                                            ...
                                            sqlInsertV = sqlInsertV & ", ("& session("User") &", "& mydatenull(Data) &", "& mytime(Hora) &", 'V', "& treatvalzero(ProfissionalID) &", "& replace(Especialidades, "|", "") &", "& treatvalzero(LocalID) &", "& treatvalzero(UnidadeID) &", "& Horarios("TipoGrade") &", "& Horarios("id") &")"
                                        ...
                                        if instr(txtHorarios, ",") then
                                            ...
                                            for ih=0 to ubound(splHorarios)
                                                ...
                                                sqlInsertV = sqlInsertV & ", ("& session("User") &", "& mydatenull(Data) &", "& mytime(HoraPers) &", 'V', "& treatvalzero(ProfissionalID) &", "& replace(Especialidades, "|", "") &", "& treatvalzero(LocalID) &", "& treatvalzero(UnidadeID) &", "& Horarios("TipoGrade") &", "& Horarios("id") &")"
                                                end if
                                            next
                                        end if
                                    end if
                            ...
                            if sqlInsertV<>"" then
                                ...
                                db.execute("insert into rel_ocupacao (sysUser, Data, Hora, Situacao, ProfissionalID, EspecialidadeID, LocalID, UnidadeID, TipoGrade, GradeID) VALUES "& sqlInsertV)
                            end if
    ...
    HoraDe = bloq("HoraDe")
    HoraA = bloq("HoraA")
        db.execute("UPDATE rel_ocupacao SET Situacao='B' WHERE sysUser="& session("User") &" AND Data="& mydatenull(Data) &" AND ProfissionalID="& treatvalnull(ProfissionalID) &" AND Hora BETWEEN "& mytime(HoraDe) &" AND "& mytime(HoraA) &" AND Situacao IN('V', 'A')")
