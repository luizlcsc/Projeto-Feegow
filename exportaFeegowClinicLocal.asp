...
origem.Open "Driver={MySQL ODBC 5.3 ANSI Driver};Server=localhost;Database="&dbOrigem&";uid="&objSystemVariables("FC_MYSQL_USER")&";pwd="&objSystemVariables("FC_MYSQL_PASSWORD")&";"
...
destino.Open "Driver={MySQL ODBC 5.3 ANSI Driver};Server=localhost;Database="&dbDestino&";uid="&objSystemVariables("FC_MYSQL_USER")&";pwd="&objSystemVariables("FC_MYSQL_PASSWORD")&";"
...
'não tinha Peso, Altura, IMC no primeiro clinic
...
'	sql = "INSERT INTO `pacientes` (`id`, `NomePaciente`, `Nascimento`, `Sexo`, `Cep`, `Cidade`, `Estado`, `Endereco`, `Numero`, `Complemento`, `Bairro`, `EstadoCivil`, `CorPele`, `GrauInstrucao`, `Profissao`, `Naturalidade`, `Tel1`, `Documento`, `Origem`, `Email1`, `CPF`, `Tabela`, `Observacoes`, `Pendencias`, `Foto`, `Religiao`, `Tel2`, `Cel1`, `Cel2`, `Email2`, `Pais`, `IndicadoPor`, `sysActive`, `sysUser`, ConvenioID1, PlanoID1, Matricula1, Titular1, Validade1) VALUES ("&p("id")&", '"&rep(p("Nome"))&"', "&nascimento(p("Nascimento"))&", '"&sexo(p("Sexo"))&"', '"&p("Cep")&"', '"&p("Cidade")&"', '"&p("Estado")&"', '"&p("Endereco")&"', '"&p("Numero")&"', '"&p("Complemento")&"', '"&p("Bairro")&"', "&treatvalzero(p("EstadoCivil"))&", "&treatvalzero(p("CorPele"))&", "&treatvalzero(p("Escolaridade"))&", '"&p("Profissao")&"', '"&p("Naturalidade")&"', '"&phone(p("DDDRes"), p("TelRes1"))&"', '"&p("Documento")&"', '"&p("Origem")&"', '"&p("Email1")&"', '"&p("CPF")&"', 0, '"&p("Obs")&"', '"&p("Pendencias")&"', '"&p("Imagem")&"', '"&p("Religiao")&"', '"&phone(p("DDDRes"), p("TelRes2"))&"', '"&phone(p("DDDCel"), p("TelCel1"))&"', '"&phone(p("DDDCel"), p("TelCel2"))&"', '"&p("Email2")&"', NULL, '"&p("IndicadoPor")&"', 1, 1, "&treatvalnull(p("Convenio1"))&", "&treatvalnull(p("ConPlano1"))&", '"&p("ConMatCar1")&"', '"&p("ConTitular1")&"', "&mydatenull(p("ConValidade1"))&")"
'	response.Write(sql)
'	destino.execute(sql)
'p.movenext
'wend
...
set p = origem.execute("select * from doutor where Ativo='S'")
while not p.eof
	destino.execute("insert into profissionais (id, Ativo, NomeProfissional, Sexo, TratamentoID, Conselho, UFConselho, sysActive, sysUser, Cor) values ("&p("id")&", 'on', '"&rep(p("Nome"))&"', '"&sexo(p("Sexo"))&"', 2, '1', '', 1, 1, '"&rep(p("Cor"))&"')")
p.movenext
wend
p.close
set p = nothing
