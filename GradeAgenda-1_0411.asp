...
<table class="table table-striped table-hover table-condensed table-agenda">
     ...
     <%
        DiaSemana = weekday(Data)
        Hora = cdate("00:00")
		set Horarios = db.execute("select ass.*, l.NomeLocal, '0' TipoGrade from assperiodolocalxprofissional ass LEFT JOIN locais l on l.id=ass.LocalID where ass.ProfissionalID="&ProfissionalID&" and DataDe<="&mydatenull(Data)&" and DataA>="&mydatenull(Data)&" order by HoraDe")
		if Horarios.EOF then
	        set Horarios = db.execute("select ass.*, l.NomeLocal from assfixalocalxprofissional ass LEFT JOIN locais l on l.id=ass.LocalID where ass.ProfissionalID="&ProfissionalID&" and ass.DiaSemana="&DiaSemana&" order by ass.HoraDe")
		end if
        if Horarios.eof then
            %>
            ...
            if Horarios("TipoGrade")=0 then
                ...
                while Hora<=HoraA
                    ...
                    <%
					    ...
					    HLivres = HLivres+1
                    %>
                    <tr onclick="abreAgenda('<%=HoraID%>', 0, '<%=Data%>', <%=LocalID%>, <%=ProfissionalID%>)" class="l l<%= LocalID %> vazio" data-hora="<%=formatdatetime(Hora, 4)%>" id="<%=HoraID%>">
                        <td width="1%"></td>
                        <td width="1%"><button type="button" class="btn btn-xs btn-info"><%= formatdatetime(Hora,4) %></button></td>
                ...
                if instr(txtHorarios, ",") then
                    ...
                    for ih=0 to ubound(splHorarios)
                        ...
                        if isdate(HoraPers) then
                            ...
                            %>
                            <tr onclick="abreAgenda('<%= HoraID %>', 0, '<%=Data%>', <%=LocalID%>, <%=ProfissionalID%>)" class="l l<%= LocalID %> vazio" data-hora="<%=formatdatetime(HoraPers, 4)%>" id="<%=HoraID%>">
                                <td width="1%"></td>
      ...
      %>
        <tr class="hidden l" id="2359"></tr>
                ...
                <script>
                <%
                set comps=db.execute("select a.id, a.Data, a.Hora, a.LocalID, a.ProfissionalID, a.StaID, a.Encaixe, a.Tempo, a.FormaPagto, p.NomePaciente, p.Tel1, p.Cel1, proc.NomeProcedimento,proc.Cor, s.StaConsulta, a.rdValorPlano, a.ValorPlano, c.NomeConvenio, l.NomeLocal, (select Resposta from agendamentosrespostas where AgendamentoID=a.id limit 1) Resposta from agendamentos a "&_
                ...
                while not comps.EOF
					...
					Conteudo = Conteudo & "</td>"&_
					"<td class=""text-center hidden-xs"" "& linkAg &"><span class=""nomePac"">"&CorProcedimento&replace(comps("NomeProcedimento")&" ", "'", "\'")&"</span></td>"&_
					"<td class=""text-center hidden-xs"" "& linkAg &">"&comps("StaConsulta")&"</td>"&_
					"<td class=""text-right nomeConv hidden-xs"" "& linkAg &">"& sinalAgenda(FormaPagto) &"&nbsp;"&replace(Valor&" ", "'", "\'")&"</td>"&_ 
					"</tr>"
