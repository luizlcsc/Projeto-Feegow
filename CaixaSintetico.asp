...
<form id="frmfiltros">
    ...
    <div class="row hidden-print">
        <h3 class="col-md-6">
			...
			%>
        <small>&raquo;  <a class="btn btn-xs btn-default" href="#" onClick="$('#filtros').toggleClass('hidden')">mais filtros</a></small></h3>
        <div class="col-xs-4">
        	<div class="row">
                <%=quickField("datepicker", "De", "De", 6, De, "", "", "")%>
            	<%=quickField("datepicker", "Ate", "Até", 6, Ate, "", "", "")%>
            </div>
        </div>
        <div class="col-xs-2">
        	<label>&nbsp;</label><br>
        	<button id="btnFiltrar" class="btn btn-sm btn-primary"><i class="fa fa-search"></i> Gerar</button>
            <button type="button" class="btn btn-sm btn-info pull-right" onclick="print()"><i class="fa fa-print"></i></button>
        </div>
    </div>
	...
	<div class="row">
	  	<div class="col-xs-12">
			<label><input class="todos" type="checkbox" value="1" ><span class="lbl">Marcar / Desmarcar todos</span></label><br>
			</div>
		</div>
      <div class="row">
        <%=quickField("empresaMultiCheck", "U", "Unidade", 12, req("U"), "", "", "")%>
      </div>
...
for r=0 to ubound(splTipoCD)
	...
	sql = "SELECT m.* FROM sys_financialmovement m WHERE m.Date BETWEEN "&mydatenull(De)&" AND "&mydatenull(Ate)&" AND Type='Pay' AND CD='"&TipoCD&"' AND UnidadeID in ("&replace(req("U"), "|", "")&") AND m.Type!='Transfer' " & sqlCx
	'response.Write(sql)
	...
	while not dist.EOF
		...
		%>
		<tr<%if dist("MetodoID")<>2 and dist("MetodoID")<>8 then%> class="pointer" onClick="ajxAfter('CaixaDetalhes', '&U=<%=req("U")%>&MetodoID=<%=dist("MetodoID")%>', 1, 'div_<%=TipoCD & dist("MetodoID")%>')"<%end if%> id="div_<%=TipoCD & dist("MetodoID")%>">
		...
		if dist("MetodoID")=8 then
			...
			while not band.eof
				...
				while not parcs.EOF
					%>
					<tr onClick="ajxAfter('CaixaDetalhes', '&U=<%=req("U")%>&MetodoID=<%=dist("MetodoID")%>&BandeiraID=<%=band("BandeiraID")%>&Parcelas=<%=parcs("Parcelas")%>', 1, 'div_<%=TipoCD & dist("MetodoID")%>_<%=band("BandeiraID")%>_<%=parcs("Parcelas")%>')" class="pointer" id="div_<%=TipoCD & dist("MetodoID")%>_<%=band("BandeiraID")%>_<%=parcs("Parcelas")%>">
						<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		...
		elseif dist("MetodoID")=2 then
			...
			while not dc.EOF
				...
				%>
				<tr id="div_<%=TipoCD & dist("MetodoID")%>_<%=dc("DiasCheque")%>" onClick="ajxAfter('CaixaDetalhes', '&U=<%=req("U")%>&MetodoID=<%=dist("MetodoID")%>&DiasCheque=<%=dc("DiasCheque")%>', 1, 'div_<%=TipoCD & dist("MetodoID")%>_<%=dc("DiasCheque")%>')" class="pointer">
					<td>&nbsp;&nbsp;&nbsp;&nbsp;
							<%=DescCq%></td>
...
<table class="table table-bordered table-hover table-condensed">
    ...
    <tbody>
    <%
	set gc = db.execute("select ConvenioID, SUM(ValorProcedimento) Total, c.NomeConvenio FROM tissguiaconsulta g LEFT JOIN convenios c on c.id=g.ConvenioID WHERE DataAtendimento BETWEEN "&mydatenull(De)&" AND "&mydatenull(Ate)&" AND UnidadeID in ("&replace(req("U"), "|", "")&") IS NOT NULL GROUP BY ConvenioID ORDER BY NomeConvenio")
	if not gc.EOF then
		set gTot = db.execute("select SUM(ValorProcedimento) Total FROM tissguiaconsulta WHERE DataAtendimento BETWEEN "&mydatenull(De)&" AND "&mydatenull(Ate)&" AND UnidadeID in ("&replace(req("U"), "|", "")&")")
