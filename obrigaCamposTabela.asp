          ...
          <tr>
            ...
            <td><%
                ...
                elseif perm("Tipo")="Agendamento" then
                    ...
                    call quickfield("multiple", "Obrigar"&perm("id"), "Campos obrigatórios", 12, perm("Obrigar"), "select label,ColumnName id from cliniccentral.sys_resourcesfields where resourceId=1 AND columnName IN ("&camposPacienteAgendaObrigar&") union all select 'IndicadoPorSelecao' id, 'Profissional Solicitante' label ", "label", "")
                    call quickfield("multiple", "Exibir"&perm("id"), "Exibir campos adicionais", 12, perm("Exibir"), "select label,ColumnName id from cliniccentral.sys_resourcesfields where resourceId=1 AND columnName IN ("&camposPacienteAgenda&") union all select 'IndicadoPorSelecao' id, 'Profissional Solicitante' label ", "label", "")
                end if
                %></td>
