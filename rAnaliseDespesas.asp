...
%>
<h3 class="text-center">Análise de Despesas</h3>
...
<table class="table table-bordered table-hover" width="100%">
    ...
    <tbody>
		...
		sql = "SELECT m.id, m.InvoiceID, m.Value, m.`Date`, m.CD, cat.Name, i.CompanyUnitID, ii.Tipo, ii.Quantidade, ii.CategoriaID, (select id from sys_financialexpensetype where Category=cat.id) inferior FROM sys_financialmovement m LEFT JOIN sys_financialinvoices i on i.id=m.InvoiceID LEFT JOIN itensinvoice ii on ii.InvoiceID=i.id LEFT JOIN sys_financialexpensetype cat on cat.id=ii.CategoriaID WHERE Type='Bill' AND m.CD='D' AND i.sysActive=1 AND m.Value>0 AND m.Date BETWEEN "&mydatenull(DataDe)&" AND "&mydatenull(DataAte)&" and i.CompanyUnitID="&splU(i)&" ORDER BY cat.Name"
		...
		while not G.eof
			...
			if UltimaCategoria<>"" and UltimaCategoria<>G("CategoriaID") then
			...
			<tr class="danger">
				...
				<td class="text-right" id="cat<%=G("CategoriaID")%>"></td>
			</tr>
			<%
			end if
			
			if UltimaCategoria<>G("CategoriaID") then
				strCatsIDDist = strCatsIDDist&"|^"&g("CategoriaID")
				strCatsNomeDist = strCatsNomeDist&"|^"&g("Name")' &  "("&g("Inferior")&")"
			end if
			if UltimaCategoria="" or UltimaCategoria=G("CategoriaID") then
				Valor = Valor+g("Value")
			end if
			if UltimaCategoria<>"" and UltimaCategoria<>G("CategoriaID") then
				Valor = g("Value")
