    ...
    select case fieldType
		...
		case "email", "phone", "mobile"
            ...
            <div class="input-group">
                ...
                <%else %>
                <input type="tel" placeholder="" value="<%=fieldValue%>" class="form-control <%=sqlOrClass%>" name="<%=fieldName%>" id="<%=fieldName%>" maxlength="150"<%=additionalTags%> />
                <%end if %>
            </div>
			<%
		case "memo"
...
function googleCalendar(Acao, Email, AgendamentoID, ProfissionalID, NomePaciente, Data, Hora, Tempo, NomeProcedimento, Notas)
	...
	if NomePaciente="" and Email<>"vca" and Acao<>"X" and isnumeric(AgendamentoID) and AgendamentoID<>"" then
		...
		if not dadosAge.EOF then
			...
			Data = dadosAge("Data")
			if session("Banco") = "clinic3776" or session("Banco") = "clinic3858" then
			    Hora = dadosAge("Hora")
             else
                Hora = dateadd("h",-1,dadosAge("Hora"))
			end if
...
private function geraRecorrente(i)
    if i=0 then
        sql = "select * from invoicesfixas where PrimeiroVencto<=date(now()) and (isnull(RepetirAte) or RepetirAte>=date(now())) and sysActive=1"
    else
        sql = ""
    end if
...
private function statusPagto(AgendamentoID, PacienteID, Data, rdValorPlano, ValorPlano, StaID, ProcedimentoID, ProfissionalID)
    ...
    if AgendamentoID="" or AgendamentoID="0" then
...
'                aexecutar = rdValorPlano &"|"& ProcedimentoID & "|" & ConvenioID &"|"& fn(Valor) & "|5_"& ProfissionalID &";"
                aexecutar = rdValorPlano &"|"& ProcedimentoID & "|" & ConvenioID &"|"& fn(Valor) &";"
            else
'                executados = rdValorPlano &"|"& ProcedimentoID & "|" & ConvenioID &"|"& fn(Valor) & "|5_"& ProfissionalID &";"
                executados = rdValorPlano &"|"& ProcedimentoID & "|" & ConvenioID &"|"& fn(Valor) &";"
            end if
        end if
        ...
        while not ate.eof
            ...
            if Valor>0 then
'                executados = executados & rdValorPlano &"|"& ProcedimentoID & "|" & ConvenioID &"|"& fn(Valor) & "|5_"& ProfissionalID &";"
                if instr(executados, rdValorPlano &"|"& ProcedimentoID & "|" & ConvenioID &"|"& fn(Valor) &";")=0 then
                    executados = executados & rdValorPlano &"|"& ProcedimentoID & "|" & ConvenioID &"|"& fn(Valor) &";"
                end if
            end if
        ate.movenext
        wend
        ...
        if instr(aexecutar, "V|") or instr(executados, "V|") then
            HaParticular = 1
            set ii = db.Execute("select ii.ItemID ProcedimentoID, ii.ProfissionalID, ifnull(ii.Quantidade * (ii.ValorUnitario+ii.Acrescimo-ii.Desconto), 0) ValorTotal, ifnull((select sum(Valor) from itensdescontados where ItemID=ii.id), 0) ValorPago, ii.Associacao from itensinvoice ii LEFT JOIN sys_financialinvoices i on i.id=ii.InvoiceID where ProfissionalID="&treatvalnull(ProfissionalID)&" and DataExecucao="& mydatenull(Data) &" and i.AccountID="&treatvalnull(PacienteID)&" and i.AssociationAccountID=3")
            while not ii.eof
                if ii("ValorPago")>=ii("ValorTotal") then
'                    strItemPago = "V|"& ii("ProcedimentoID") &"|0|"& fn(ii("ValorTotal")) &"|"& ii("Associacao") &"_"& ii("ProfissionalID") &";"
                    strItemPago = "V|"& ii("ProcedimentoID") &"|0|"& fn(ii("ValorTotal")) &";"
                    aexecutar = replace( aexecutar, strItemPago, "", 1, 1 )
                    executados = replace( executados, strItemPago, "", 1, 1 )
                end if
            ii.movenext
            wend
            ii.close
        ...
        if instr(aexecutar, "P|") or instr(executados, "P|") then
            ...
            while not gcons.eof
                if isnull(gcons("ProfissionalEfetivoID")) or gcons("ProfissionalEfetivoID")=0 then
                    ...
                    ProfissionalID = gcons("ProfissionalEfetivoID")
                end if
'                strItemPago = "P|"& gcons("ProcedimentoID") &"|"& gcons("ConvenioID") &"|"& fn(gcons("ValorProcedimento")) &"|"& "5_"& ProfissionalID &";"
                strItemPago = "P|"& gcons("ProcedimentoID") &"|"& gcons("ConvenioID") &"|"& fn(gcons("ValorProcedimento")) &";"
                aexecutar = replace( aexecutar, strItemPago, "", 1, 1 )
                executados = replace( executados, strItemPago, "", 1, 1 )
            gcons.movenext
            wend
    ...
    select case FM
        case "F"
            if ContaVariavel="S" then
                %>
                <div class="col-md-3">
                    <%=selectInsertCA(FuncaoID &". "& Funcao&"&nbsp;", "C"& NomeCampo, ContaPadrao, "5, 4, 3, 2, 6, 1", "", "", "") %>
                </div>
...
private function calcValPosicao(QuantidadeAtual, ValorAtual, QuantidadeInserida, ValorInserido)
    ...
    calcValPosicao = ( (QuantidadeAtual*ValorAtual) + (QuantidadeInserida*ValorInserido) ) / (QuantidadeAtual + QuantidadeInserida)
    if 1=2 then
        %>
...
private function LanctoEstoque(LancamentoID, PosicaoID, P, Tipo, TipoUnidade, Quantidade, Data, FornecedorID, Lote, Validade, NotaFiscal, Valor, UnidadePagto, Observacoes, Responsavel, PacienteID, Lancar, LocalizacaoID, ItemInvoiceID, AtendimentoID, tipoResultado, CBID, ProdutoInvoiceID)
    ...
    if ItemInvoiceID<>"" then
        set ii = db.execute("select * from itensinvoice where id="& ItemInvoiceID)
        if not ii.eof then
        ...
        UnidadesLancadas = 0
        set lan = db.Execute("select * from estoquelancamentos where ItemInvoiceID="& ItemInvoiceID)
        while not lan.eof
    ...
    if erro<>"" and tipoResultado<>"ignore" then
        ...
        if tipoResultado<>"ignore" then
	        db_execute("insert into estoquelancamentos (ProdutoID, EntSai, Quantidade, TipoUnidade, Data, FornecedorID, Lote, Validade, NF, Valor, UnidadePagto, Observacoes, Responsavel, PacienteID, Lancar, sysUser, QuantidadeConjunto, QuantidadeTotal, LocalizacaoID, ItemInvoiceID, AtendimentoID, posicaoAnte, ProdutoInvoiceID) values ("&P&", '"&Tipo&"', "&treatvalzero(Quantidade)&", '"&TipoUnidade&"', "&mydatenull(Data)&", '"& FornecedorID &"', '"&Lote&"', "&mydatenull(Validade)&", '"&NotaFiscal&"', "&treatvalzero(Valor)&", '"&UnidadePagto&"', '"&Observacoes&"', '"&Responsavel&"', "& treatvalnull(PacienteID) &", '"& Lancar &"', "&session("User")&", "&treatvalzero(QuantidadeConjunto)&", "&treatvalzero(UnidadesTentadas)&", "& treatvalzero( LocalizacaoID ) &", "& treatvalnull( ItemInvoiceID ) &", "& treatvalnull( AtendimentoID ) &", (select group_concat( concat(id, '=', concat(Quantidade, '|', ValorPosicao)) SEPARATOR ', ') from estoqueposicao where ProdutoID="& P &" and Quantidade<>0), "& treatvalnull(ProdutoInvoiceID) &")")
        ...
        if Tipo="E" then
            ...
            'if TipoUnidade="C" then
                ...
                sqlVCA = "select * from estoqueposicao where ProdutoID="&P&" AND TipoUnidade='"&TipoUnidade&"' AND Lote like '"&Lote&"' AND LocalizacaoID LIKE '"& LocalizacaoID &"' AND CBID LIKE '"& CBID &"' AND Responsavel like '"&Responsavel&"'"& sqlValidade
        ...
        elseif Tipo="S" or Tipo="M" then
            PosicaoE = 0
            if isnull(PosicaoID) then 'ou seja, eh relancamento
                if Validade="" or isnull(Validade) then
                    sqlValidade = " AND ISNULL(Validade) "
                else
                    sqlValidade = " AND Validade="&mydatenull(Validade)
                end if
                'function lancar(P, T, L, V) - do antigo
                sqlPosSaida = "select id from estoqueposicao where ProdutoID like '"& P &"' and TipoUnidade like '"& TipoUnidade &"' and Lote like '"& Lote &"' "& sqlValidade
                response.write( sqlPosSaida &"<br>")
                set posSaida = db.execute( sqlPosSaida )
                if not posSaida.eof then
                    PosicaoID = posSaida("id")
                else
                    sqlPosSaidaU = "select id from estoqueposicao where ProdutoID like '"& P &"' and (TipoUnidade like 'U' or TipoUnidade like '') and Lote like '"& Lote &"' "& sqlValidade
                    set posSaidaU = db.execute( sqlPosSaidaU )
                    if not posSaidaU.eof then
                        PosicaoID = posSaidaU("id")
                    end if
                end if
            end if
            set pos = db.execute("select * from estoqueposicao where id="& PosicaoID)
            if not pos.eof then
                if pos("TipoUnidade")=TipoUnidade then
                    ValorComprado = pos("ValorPosicao")
                elseif pos("TipoUnidade")="C" and TipoUnidade="U" then
                    ValorComprado = pos("ValorPosicao") / ApresentacaoQuantidade
'                elseif pos("TipoUnidade")="U" and TipoUnidade="C" then -> Não dá pra tirar conjunto pela unidade
'                    ValorComprado = pos("ValorPosicao") * ApresentacaoQuantidade
                end if
                db_execute("update estoquelancamentos SET ValorComprado="& treatvalzero( ValorComprado ) &" WHERE id="& LancamentoID)
                ApresentacaoQuantidade = prod("ApresentacaoQuantidade")
                ValorUnidade = pos("ValorPosicao")/ApresentacaoQuantidade
                if (TipoUnidade="C" and pos("TipoUnidade")="C") or (TipoUnidade="U" and pos("TipoUnidade")="U") then
                    db_execute("update estoqueposicao set Quantidade=(Quantidade-"&treatvalzero(Quantidade)&") where id="&pos("id"))
                elseif pos("TipoUnidade")="C" and TipoUnidade="U" then
                    QuantidadeUniRetirada = ccur(Quantidade)
                    QuantidadeConAtual = pos("Quantidade")
                    if QuantidadeUniRetirada<=ApresentacaoQuantidade then
                        QuantidadeConNova = QuantidadeConAtual - 1
                        QuantidadeUniSobra = ApresentacaoQuantidade - QuantidadeUniRetirada
                    else
                        QuantidadesInteiras = cint(QuantidadeUniRetirada / ApresentacaoQuantidade)
                        if (QuantidadesInteiras*ApresentacaoQuantidade)>QuantidadeUniRetirada then
                            QuantidadesInteiras = QuantidadesInteiras-1
                        end if
                        QuantidadeConNova = QuantidadeConAtual - QuantidadesInteiras
    '                    QuantidadeUniSobra = QuantidadeUniRetirada - (QuantidadesInteiras * ApresentacaoQuantidade)
                        QuantidadeUniSobra = ApresentacaoQuantidade - (QuantidadeUniRetirada - (QuantidadesInteiras * ApresentacaoQuantidade))
    '                    response.write(QuantidadeUniSobra&" = "&QuantidadeUniRetirada&" - ("&QuantidadesInteiras&" * "&ApresentacaoQuantidade&")")
                    end if
                    if QuantidadeUniSobra>0 then
                        if QuantidadeUniRetirada>ApresentacaoQuantidade then
                            QuantidadeConNova = QuantidadeConNova-1
                        end if
                        if isnull(pos("Validade")) then
                            sqlValidade = " AND ISNULL(Validade) "
                        else
                            sqlValidade = " AND Validade="&mydatenull(pos("Validade"))
                        end if
                        set vca = db.execute("select id, Quantidade, ValorPosicao from estoqueposicao where ProdutoID="&pos("ProdutoID")&" and TipoUnidade='U' and Responsavel='"&pos("Responsavel")&"' and CBID='"&pos("CBID")&"' and LocalizacaoID="&treatvalzero(pos("LocalizacaoID"))&" and Lote='"&pos("Lote")&"'"& sqlValidade)
                        'AQUI ENTRA A SOBRA DE UMA SAÍDA OU INDIVIDUALIZAÇÃO (verificar ainda)
                        if vca.eof then'or ref("Individualizar")="S" then
                            ValorPosicao = calcValPosicao(0, 0, QuantidadeUnisobra, ValorUnidade)
                            sqlIns = "insert into estoqueposicao (ProdutoID, Quantidade, TipoUnidade, Responsavel, CBID, LocalizacaoID, Lote, Validade, ValorPosicao) select ProdutoID, "&treatvalzero(QuantidadeUniSobra)&", 'U', Responsavel, '"& CBID &"', LocalizacaoID, Lote, Validade, "& treatvalzero(ValorPosicao) &" FROM estoqueposicao WHERE id="& PosicaoID
                            'response.write( sqlIns )
                            db.execute( sqlIns )
                        else
                            ValorPosicao = calcValPosicao(vca("Quantidade"), vca("ValorPosicao"), QuantidadeUnisobra, ValorUnidade)
                            db_execute("update estoqueposicao set Quantidade=Quantidade+"& treatvalzero(QuantidadeUniSobra) &", ValorPosicao="& treatvalzero(ValorPosicao) &" where id="& vca("id"))
                        end if
                        set pultEnt = db.execute("select id from estoqueposicao order by id desc limit 1")
                        PosicaoE = pultEnt("id")
                    end if
                    db_execute("update estoqueposicao SET Quantidade="& treatvalzero(QuantidadeConNova) &" WHERE id="& PosicaoID)
                end if
                'se for movimentacao, também cria a linha do que o usuario movimentou
                if Tipo="M" then
                    if TipoUnidade="C" then
                        ValorNovo = pos("ValorPosicao")
                    else
                        ValorNovo = ValorUnidade
                    end if
                    if ref("Individualizar")="" then 'somente movimentou sem individualizar
                        if isnull(pos("Validade")) then
                            sqlValidade = " AND ISNULL(Validade) "
                        else
                            sqlValidade = " AND Validade="&mydatenull(pos("Validade"))
                        end if
                        set vca = db.execute("select * from estoqueposicao where ProdutoID="&P&" AND TipoUnidade='"&TipoUnidade&"' AND Lote like '"&pos("Lote")&"' AND LocalizacaoID="& treatvalzero(LocalizacaoID) &" AND CBID LIKE '"& CBID &"' AND Responsavel like '"& Responsavel &"' "& sqlValidade)
                        if not vca.eof then
                            ValorNovo = calcValPosicao(vca("Quantidade"), vca("ValorPosicao"), Quantidade, pos("ValorPosicao"))
                            db_execute("update estoqueposicao set Quantidade=(Quantidade+"&treatvalzero(Quantidade)&") where id="&vca("id"))
                            PosicaoE = vca("id")
                        else
                            db_execute("insert INTO estoqueposicao (ProdutoID, Quantidade, TipoUnidade, Responsavel, CBID, LocalizacaoID, Lote, Validade, ValorPosicao) VALUES ("&P&", "& treatvalzero(Quantidade) &", '"& TipoUnidade &"', '"& Responsavel &"', '"& CBID &"', "& treatvalzero( LocalizacaoID ) &", '"& pos("Lote") &"', "& mydatenull(pos("Validade")) &", "& treatvalzero( pos("ValorPosicao") ) &")")
                        end if
                    else'Individualizou
                        ...
                        while cInd<tInd
                            cInd = cInd+1
                            sqlVCAInd = "select * from estoqueposicao where ProdutoID="&P&" AND TipoUnidade='"&TipoUnidade&"' AND Lote like '"&pos("Lote")&"' AND LocalizacaoID="& treatvalzero(LocalizacaoID)&" AND CBID LIKE '"& ref("ind"& cInd) &"' AND Responsavel like '"& Responsavel &"' "& sqlValidade
                            'response.write("//"& sqlVCAInd )
                            set vcaInd = db.execute( sqlVCAInd )
                            if vcaInd.eof then
                                db_execute("insert INTO estoqueposicao (ProdutoID, Quantidade, TipoUnidade, Responsavel, CBID, LocalizacaoID, Lote, Validade, ValorPosicao) VALUES ("&P&", "& 1 &", '"& TipoUnidade &"', '"& Responsavel &"', '"& ref("ind"& cInd) &"', "& treatvalzero( LocalizacaoID ) &", '"& pos("Lote") &"', "& mydatenull( pos("Validade") ) &", "& treatvalzero( ValorNovo ) &")")
                            else
        ...
        if tipoResultado<>"ignore" then
            db_execute("update estoquelancamentos set PosicaoS="& PosicaoS &", PosicaoE="& PosicaoE &" WHERE id="& LancamentoID)
        end if
        if tipoResultado="eval" then
            ...
            if Lancar="S" then
                call estoqueLancaConta(LancamentoID, "nothing")
            end if
            call atualizaPosicao(P)
        end if
    end if    
end function

private function estoqueLancaConta(LancamentoID, resultado)
        ...
        <div class="btn-group">
            ...
            <ul class="dropdown-menu" role="menu">
                ...
                while not cc.eof
                    %>
                    <li>
                        <a href="javascript:btb(<%=cc("id") %>, '<%= Numero %>', '3_<%=Contato %>')"><i class="fa fa-<%=cc("Icone") %>"></i> <%=cc("NomeCanal") %></a>
                    </li>
                    <%
                cc.movenext
                wend
...
private function FazPosicao(ProdutoID)
    ...
    while not lanctos.eof
        ...
        call LanctoEstoque(lanctos("id"), PosicaoID, ProdutoID, lanctos("EntSai"), lanctos("TipoUnidade"), lanctos("Quantidade"), lanctos("Data"), lanctos("FornecedorID"), lanctos("Lote"), lanctos("Validade"), lanctos("NF"), lanctos("Valor"), lanctos("UnidadePagto"), lanctos("Observacoes"), lanctos("Responsavel"), lanctos("PacienteID"), lanctos("Lancar"), lanctos("LocalizacaoID"), lanctos("ItemInvoiceID"), lanctos("AtendimentoID"), "ignore", lanctos("CBID"), lanctos("ProdutoInvoiceID"), lanctos("Individualizar"))
    lanctos.movenext
    wend
    lanctos.close
    set lanctos = nothing
...
private function atualizaPosicao(ProdutoID)
    set poe = db_execute("update produtos p SET p.posicaoConjunto=IFNULL((select sum(Quantidade) from estoqueposicao where ProdutoID=p.id and TipoUnidade='C'), 0), p.posicaoUnidade=IFNULL((select sum(Quantidade) from estoqueposicao where ProdutoID=p.id and TipoUnidade='U'), 0) WHERE p.id="& ProdutoID)
end function
