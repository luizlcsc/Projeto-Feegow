...
if request.QueryString("X")<>"" then
	db_execute("delete from cirurgiaanexos where id="&request.QueryString("X"))
end if
%>
<table width="100%" class="table table-striped table-bordered table-condensed">
  <thead>
    <tr>
      ...
      <th width="30" align="center" nowrap> <button type="button" class="btn btn-info btn-xs" onClick="itemCirurgia('Despesas', <%=request.QueryString("I")%>, 0);"><i class="fa fa-plus"></i></button></th>
    </tr>
  </thead>
  <tbody>
  <%
  set p = db.execute("select * from cirurgiaanexos where GuiaID="&request.QueryString("I"))
  while not p.eof
  %>
    <tr id="lDespesas<%=p("id") %>">
      <td align="center"><button type="button" class="btn btn-xs btn-success" onClick="itemCirurgia('Despesas', <%=request.QueryString("I")%>, <%=p("id")%>);"><i class="fa fa-edit"></i></button></td>
