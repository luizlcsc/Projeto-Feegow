...
'livenote
	...
	cl=0
	tot = 0
	sql = "select distinct sms.LicencaID, lic.Cupom, lic.NomeContato from smshistorico sms LEFT JOIN licencas lic ON lic.id = sms.LicencaID WHERE date(sms.DataHora)>="&mydatenull(req("De"))&" AND date(sms.DataHora)<="&mydatenull(req("Ate"))
	set lic = dbc.execute(sql)
	
	while not lic.eof
		NomeCliente = ""

		set conta = dbc.execute("select count(id) as total from smshistorico WHERE LicencaID="&lic("LicencaID")&" AND date(DataHora)>="&mydatenull(req("De"))&" AND date(DataHora)<="&mydatenull(req("Ate")))
		cl = cl+1
