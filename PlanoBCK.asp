...
<form id="frmfiltros">
    ...
    <div class="clearfix form-actions" id="filtros">
      <div class="row">
...
for i=0 to ubound(spl)
	...
	if spl(i)="RECEITAS" then
		...
		set movs = db.execute("select m.*, (select count(id) from sys_financialmovement WHERE InvoiceID=m.InvoiceID) Parcelas, (select SUM(Value) from sys_financialmovement WHERE InvoiceID=m.InvoiceID) TotalInvoice from sys_financialmovement m WHERE month(m.Date)="&Mes&" AND year(m.Date)="&Ano&" AND m.CD='D' AND m.Type='Bill'")
		while not movs.eof
...
'				response.Write("- &gt; Total do item: "&ValorItem&" - Percentual item: "&PercItem&" - Valor Proporcional: "&ValorProp&" - Categoria: "&itens("CategoriaID")&"<br>")
	...
	while not n1.EOF
		...
		%>
		<tr<%if (valRecursivo("cTotal")=0 or isnull(valRecursivo("cTotal"))) AND req("Ocultar")="S" then%> class="hidden"<%end if%>>
			<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%=c0 &"."& c1%>. <%=n1("Name")%></td>
			<td class="text-right hidden">0</td>
			<td class="text-right"><%=formatnumber(0&valRecursivo("cTotal"),2)%></td>
		</tr>
