...
'				response.Write(cstr(doSql("id"))&"="&fieldValue)
		...
		case "datepicker"
            ...
            %>
                <input id="<%= fieldName %>" class="form-control input-mask-date <%=classDatePicker%> <%=sqlOrClass%>" type="text" value="<%= fieldValue %>" name="<%= fieldName %>" data-date-format="dd/mm/yyyy"<%=additionalTags%>>
            <%else
...
function macro(editor)
	...
	macro = macro&"	  <ul class=""dropdown-menu"">"
	strUni = "HoraAgendamento, DataAgendamento, HoraChegada"
	splUni = split(strUni, ", ")
	for unk=0 to ubound(splUni)
...
'			end if
	...
	if instr(valor, "[Agendamentos.")>0 then
	    ...
	    if not AgendamentoSQL.eof then
            for corre=0 to ubound(correSubs)
                valor = replace(valor, "[Agendamento."&correSubs(corre)&"]", AgendamentoSQL(""&correSubs(corre)&"") )
            next
            ...
            'response.write("oi")
                ...
                EsteItem = ""
                AgendamentosItens = ""
                set ii = db.execute("SELECT convenios.NomeConvenio, procedimentos.NomeProcedimento, agendamentos.TipoCompromissoID,agendamentos.ValorPlano,agendamentos.rdValorPlano FROM agendamentos "&_
                                     "LEFT JOIN procedimentos ON procedimentos.id=agendamentos.TipoCompromissoID "&_
                                     ...
                                     "union "&_
                                     "SELECT convenios.NomeConvenio, procedimentos.NomeProcedimento, agendamentosprocedimentos.TipoCompromissoID,agendamentosprocedimentos.ValorPlano,agendamentosprocedimentos.rdValorPlano FROM agendamentosprocedimentos "&_
                                     "LEFT JOIN procedimentos ON procedimentos.id=agendamentosprocedimentos.TipoCompromissoID "&_
	...
	if instr(valor, "[Unidade.")>0 then
		...
		camposASubs = "Nome, Endereco, Numero, Complemento, Bairro, Cidade, Estado, Tel1, Cel1, Email1, CNPJ, CNES, Coordenadas"
		correSubs = split(camposASubs, ", ")
		set registro = db.execute(sql)
		for corre=0 to ubound(correSubs)
...
'    header = header & "        <div class=""widget-toolbar""><div>"
	if recurso="sys_financialinvoices" then
        ...
        if not RecursosAdicionaisSQL.eof then
            if instr(RecursosAdicionaisSQL("RecursosAdicionais"), "|NFe|") then
...
'	set fs=nothing
 '       response.write(txt)
	db.execute(txt)
    '    response.Write("{"& txt &"}")
    db_execute("insert into cliniccentral.updates (codigoSQL, Banco, UserID) values ('"&rep(txt)&"', '"&session("Banco")&"', "& treatvalnull(session("User")) &")")
end function
...
function btnParcela(MovimentacaoID, ValorPago, Valor, Vencimento, CD, CaixaID)
	...
	btnParcela = "<div class='btn-group'><span "& spanVenc &" class='btn btn-xs btn-"&classe&" text-right'>"&txt &"</span>" & zoom&"</div>"
...
private function statusPagto(AgendamentoID, PacienteID, Data, rdValorPlano, ValorPlano, StaID, ProcedimentoID, ProfissionalID)
    ...
    'adicioneis os status de 100 + por causa da triagem/pos consulta
    if StaID=2 or StaID=3 or StaID=4 or StaID=5 or StaID=12 or StaID=101 or StaID=102 or StaID=103 or StaID=104 or StaID=105 or StaID=106 then
    ...
    if erro<>"" and tipoResultado<>"ignore" then
        ...
        if Tipo="E" then
            ...
            'if TipoUnidade="C" then
                ...
                sqlVCA = "select * from estoqueposicao where ProdutoID="&P&" AND TipoUnidade='"&TipoUnidade&"' AND Lote like '"&Lote&"' AND ifnull(LocalizacaoID, 0) LIKE "& treatvalzero(LocalizacaoID) &" AND CBID LIKE '"& CBID &"' AND Responsavel like '"&Responsavel&"'"& sqlValidade
        ...
        elseif Tipo="S" or Tipo="M" then
            ...
            if not posSaida.eof then
                ApresentacaoQuantidade = ccur(prod("ApresentacaoQuantidade"))
...
private function linhaAgenda(n, ProcedimentoID, Tempo, rdValorPlano, Valor, ConvenioID, Convenios, EquipamentoID, LocalID)
    if rdValorPlano="V" then
        ConvenioID=0
    end if
    %>
    <tr id="la<%=n %>">
        <td><%= selectInsert("", "ProcedimentoID"& n, ProcedimentoID, "procedimentos", "NomeProcedimento", " onchange=""parametros(this.id, this.value);"" data-agenda=""""", "", "") %></td>
        <td>
            <%=quickField("number", "Tempo"&n, "", 2, Tempo, "", "", " placeholder='Em minutos'")%>
        </td>
        <td>
            <div class="radio-custom radio-primary"><input type="radio" name="rdValorPlano<%=n %>" id="rdValorPlanoV<%=n %>" required value="V"<% If rdValorPlano="V" Then %> checked="checked"<% End If %> class="ace valplan" style="z-index:-1" onclick="valplan('<%=n%>', 'V')" /><label for="rdValorPlanoV<%=n %>" class="radio"> Particular</label></div>
            <%
            if Convenios<>"Nenhum" then
            %>
            <div class="radio-custom radio-primary"><input type="radio" data-n="<%=n %>" name="rdValorPlano<%=n %>" id="rdValorPlanoP<%=n %>" required value="P"<% If rdValorPlano="P" Then %> checked="checked"<% End If %> class="ace valplan" onclick="valplan('<%=n%>', 'P')" style="z-index:-1" /><label for="rdValorPlanoP<%=n %>" class="radio"> Conv&ecirc;nio</label></div>
            <%end if %>
        </td>
        <td>
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        if aut("areceberpacienteV")=1 then
                            %>
                            <%=quickField("text", "Valor"&n, "", 5, fn(Valor), " text-right input-mask-brl ", "", "")%>
                            <%
                        else
                            %>
                            <input type="hidden" id="Valor<%=n %>" name="Valor<%=n %>" value="<%=Valor%>">
                            <%
                        end if
                        %>
                    </div>
            ...
            <div class="col-md-12" id="divConvenio<%=n %>" <% If rdValorPlano<>"P" Then %> style="display:none"<% End If %>>
                ...
                %>
                <%=selectInsert("", "ConvenioID"&n, ConvenioID, "convenios", "NomeConvenio", "", "", "")%>
                <%
                else
        ...
        <td><%= selectInsert("", "LocalID"&n, LocalID, "locais", "NomeLocal", "", "", "") %></td>
        <td>
            <%if req("Tipo")="Quadro" or req("EquipamentoID")="" or req("EquipamentoID")="undefined" or req("EquipamentoID")="0" then%> 
                <%=quickfield("select", "EquipamentoID"&n, "", 2, EquipamentoID, "select * from equipamentos where sysActive=1", "NomeEquipamento", "") %>
            <%else %>
      ...
      '      else
       '         PosicaoES = lanctos("PosicaoS")
        '    end if
            ...
            call LanctoEstoque(lanctos("id"), PosicaoES, ProdutoID, lanctos("EntSai"), lanctos("TipoUnidadeOriginal"), lanctos("TipoUnidade"), lanctos("Quantidade"), lanctos("Data"), lanctos("FornecedorID"), lanctos("Lote"), lanctos("Validade"), lanctos("NF"), lanctos("Valor"), lanctos("UnidadePagto"), lanctos("Observacoes"), lanctos("Responsavel"), lanctos("PacienteID"), lanctos("Lancar"), lanctos("LocalizacaoID"), lanctos("ItemInvoiceID"), lanctos("AtendimentoID"), "ignore", lanctos("CBID"), lanctos("ProdutoInvoiceID"), lanctos("ResponsavelOriginal"), lanctos("LocalizacaoIDOriginal"), lanctos("Individualizar"), lanctos("CBIDs"))
        lanctos.movenext
        wend
        lanctos.close
        set lanctos = nothing
...
private function BaixaAutomatica(Data, ProfissionalID, ProcedimentoID, rdValorPlano, ValorPlano, PacienteID, StaID, AgendamentoID)
            ...
            if BaixaAuto="S" then
                ...
                if vcaBaixa.eof then
                    ...
                    if not vcaIIabaixar.eof then
                        ...
                        db_execute("insert into itensinvoice (InvoiceID, Tipo, Quantidade, CategoriaID, CentroCustoID, ItemID, ValorUnitario, Desconto, Descricao, Executado, DataExecucao, HoraExecucao, AgendamentoID, sysUser, ProfissionalID, HoraFim, Acrescimo, AtendimentoID, Associacao, PacoteID) values ("& InvoiceID &", 'S', 1, 0, 0, "&treatvalzero(ProcedimentoID)&", "& treatvalzero(ValorPlano) &", 0, '', 'S', "& mydatenull(Data) &", NULL, "&treatvalzero( AgendamentoID )&", "& session("User") &", "& treatvalzero(ProfissionalID) &", NULL, 0, 0, 5, 0)")
...
private function lrResult( lrStatus, lrDataExecucao, lrNomeFuncao, lrInvoiceID, lrNomeProcedimento, lrNomePaciente, lrFormaPagto, lrCreditado, lrValorProcedimento, lrValorRecebido, lrValorRepasse, lrLinha, lrFM, lrSobre )
    select case lrFM
        case "F"
...
function getNomeLocalUnidade(UnidadeID)
    if len(session("Unidades"))>3 then
...
function scp()
    if session("Banco")="clinic3882" or session("Banco")="clinic2263" then
        scp = 1
    else
        scp = 0
    end if
