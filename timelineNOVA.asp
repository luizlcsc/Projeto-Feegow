...
<div class="row">
    <div class="col-xs-12">
        <%
function show(largura, rotulo, coluna, texto)
    if instr(omissos, "|"& coluna &"|")=0 then
        if texto&""<>"" then
            response.write("<div class='col-md-"& largura &" mt10'><b>"& rotulo &"</b><br> "& texto &"</div>")
        end if
    end if
end function
...
Tipo = req("Tipo")
set PacienteSQL = db.execute("SELECT p.*, s.NomeSexo, grau.GrauInstrucao, ec.EstadoCivil, ori.Origem, cor.NomeCorPele, tab.NomeTabela FROM pacientes p LEFT JOIN sexo s ON s.id=p.Sexo LEFT JOIN grauinstrucao grau ON grau.id=p.GrauInstrucao LEFT JOIN estadocivil ec ON ec.id=p.EstadoCivil LEFT JOIN origens ori ON ori.id=p.Origem LEFT JOIN corpele cor ON cor.id=p.CorPele LEFT JOIN tabelaparticular tab ON tab.id=p.Tabela WHERE p.id = "&PacienteID)
...
<% if req("Full")="1" then
    Foto = PacienteSQL("Foto")&""

    Foto = "9d44023056cbb9404db0a89d6a127c31.jpg"

    Endereco = PacienteSQL("Endereco")
    if PacienteSQL("Numero")&""<>"" then
        Endereco = Endereco &", "& PacienteSQL("Numero")
    end if
    if PacienteSQL("Complemento")&""<>"" then
        Endereco = Endereco &", "& PacienteSQL("Complemento")
    end if
    if PacienteSQL("Bairro")&""<>"" then
        Endereco = Endereco &" - "& PacienteSQL("Bairro")
    end if
    if PacienteSQL("NomeSocial")&""<>"" then
        NomeSocial =  " ("& PacienteSQL("NomeSocial") &")"
    end if

    if PacienteSQL("Tel1")&""<>"" then
        Telefones =  " / "& PacienteSQL("Tel1")
    end if
    if PacienteSQL("Tel2")&""<>"" then
        Telefones =  Telefones & " / "& PacienteSQL("Tel2")
    end if
    if PacienteSQL("Cel1")&""<>"" then
        Telefones =  Telefones & " / "& PacienteSQL("Cel1")
    end if
    if PacienteSQL("Cel2")&""<>"" then
        Telefones =  Telefones & " / "& PacienteSQL("Cel2")
    end if
    if Telefones<>"" then
        Telefones = right(Telefones, len(Telefones)-2)
    end if

    if PacienteSQL("Email1")&""<>"" then
        Emails =  Emails & " / "& PacienteSQL("Email1")
    end if
    if PacienteSQL("Email2")&""<>"" then
        Emails =  Emails & " / "& PacienteSQL("Email2")
    end if
    if Emails<>"" then
        ...
        <div class="panel">
            <div class="panel-body">
                <div class="row">
                    <div class="col-md-2">
                        <% if Foto<>"" then %>
                        <img src="./../uploads/<%= Foto %>" class="img-thumbnail" style="max-height:170px" />
                        <% end if %>
                    </div>
                    <div class="col-md-10">
                            <%
                            ...
                            response.write("<div class='row'>")
                                call show(4, "Nome", "NomePaciente", PacienteSQL("NomePaciente") & NomeSocial)
                                call show(4, "Nascimento", "Nascimento", PacienteSQL("Nascimento") &" - "& idade(PacienteSQL("Nascimento")))
                                call show(2, "Sexo", "Sexo", PacienteSQL("NomeSexo"))
                                call show(2, "Prontuário", "id", PacienteSQL("id"))
                            'response.write("</div>")
                            'Bio
                            'response.write("<div class='row mt10'>")
                                call show(2, "Altura", "Altura", PacienteSQL("Altura"))
                                call show(2, "Peso", "Peso", PacienteSQL("Peso"))
                                call show(2, "IMC", "IMC", PacienteSQL("IMC"))
                                call show(2, "Cor da Pele", "CorPele", PacienteSQL("NomeCorPele"))
                                call show(2, "Estado Civil", "EstadoCivil", PacienteSQL("EstadoCivil"))
                            'response.write("</div>")
                            'DOC
                            'response.write("<div class='row mt10'>")
                                call show(3, "Profissão", "Profissao", PacienteSQL("Profissao"))
                                call show(3, "Escolaridade", "GrauInstrucao", PacienteSQL("GrauInstrucao"))
                                call show(2, "CPF", "CPF", PacienteSQL("CPF"))
                                call show(2, "RG", "Documento", PacienteSQL("Documento"))
                                call show(2, "CNS", "CNS", PacienteSQL("CNS"))
                            'response.write("</div>")
                            '3
                            'response.write("<div class='row mt10'>")
                                call show(8, "Endereço", "Endereco", Endereco)
                                call show(2, "Cidade / Estado", "Cidade", PacienteSQL("Cidade") &" / "& PacienteSQL("Estado"))
                                call show(2, "Cep", "Cep", PacienteSQL("Cep"))
                            'response.write("</div>")
                            'C
                            'response.write("<div class='row mt10'>")
                                call show(6, "Telefones", "Tel1", Telefones)
                                call show(6, "E-mails", "Email1", Emails)
                            'response.write("</div>")
                            'X
                            'response.write("<div class='row mt10'>")
                                call show(2, "Origem", "Origem", PacienteSQL("Origem"))
                                call show(2, "Indicado por", "IndicadoPor", PacienteSQL("IndicadoPor"))
                                call show(2, "Naturalidade", "Naturalidade", PacienteSQL("Naturalidade"))
                                call show(2, "Religião", "Religiao", PacienteSQL("Religiao"))
                                call show(2, "Tabela", "TabelaID", PacienteSQL("NomeTabela"))
                            response.write("</div>")
                            %>
                        <hr class="short alt" />
                        <div class="row">
                            <ul class="nav nav-pills mb10 pull-right">
                                <li class="active">
                                    <a href="#tab17_1" data-toggle="tab">Linha do Tempo</a>
                                </li>
                                <li>
                                    <a data-toggle="tab" class="mainTab" href="#Dados" aria-expanded="true">Ficha Principal</a>
                                </li>
                                <li>
                                    <a href="#tab17_3" data-toggle="tab">Dados Adicionais</a>
                                </li>
                            </ul>
                        </div>
                    </div>
...
%>
	...
	if instr(Tipo, "|Prescricao|")>0 then
        sqlPrescricao = " union all (select id, ControleEspecial, sysUser, 'Prescricao', 'Prescrição', 'flask', 'warning', `Data`, Prescricao from pacientesprescricoes WHERE sysActive=1 AND PacienteID="&PacienteID&") "
    end if
    ...
    if instr(Tipo, "|Atestado|")>0 then
        sqlAtestado = " union all (select id, '', sysUser, 'Atestado', ifnull(Titulo, 'Atestado'), 'file-text-o', 'success', `Data`, Atestado from pacientesatestados  WHERE sysActive=1 AND PacienteID="&PacienteID&") "
    end if

    if instr(Tipo, "|Pedido|")>0 then
        sqlPedido = " union all (select id, '', sysUser, 'Pedido', 'Pedido de Exame', 'hospital-o', 'system', `Data`, PedidoExame from pacientespedidos WHERE sysActive=1 AND PacienteID="&PacienteID&") "
...
'                response.Write( Rotulo & Valor  &"<br>{{"& ti("Tipo") &"}}" )
                select case ti("Tipo")
                    case "AE", "L"
                        ...
                        set l = db.execute("select l.StatusID from laudos l where not isnull(l.StatusID) and l.FormPID="& ti("id"))
                        if not l.eof then
                            LaudoID = l("id")
                ...
                if ti("Tipo")="Pedido" or ti("Tipo")="PedidosSADT" or ti("Tipo")="Prescricao" then
                    if ti("Tipo")="Pedido" then
                        tblRes = "pacientespedidos"
                    elseif ti("Tipo")="PedidosSADT" then
                        tblRes = "pedidossadt"
                    elseif ti("Tipo")="Prescricao" then
                        tblRes = "pacientesprescricoes"
                    end if
                    Resultado = ""
                    set vcaRes = db.execute("select Resultado from "& tblRes &" where id="& ti("id"))
                    if not vcaRes.eof then
                        Resultado = vcaRes("Resultado")
                    end if
                    if ti("sysUser")=session("User")&"" then
                    %>
                    <form method="post" id="frm<%= tblRes & ti("id") %>">
                        <textarea onclick="$(this).attr('rows', 2); $('#btn<%= tblRes & ti("id") %>').removeClass('hidden');" id="res<%= tblRes & ti("id") %>" class="form-control" placeholder="Resultado..." rows="1"><%= Resultado %></textarea>
                        <button id="btn<%= tblRes & ti("id") %>" class="btn-primary btn btn-xs hidden">SALVAR</button>
                    </form>
                    <script type="text/javascript">
                        $('#frm<%= tblRes & ti("id") %>').submit(function () {
                            $.post("saveResult.asp?I=<%= ti("id")%>&T=<%= tblRes %>", { Resultado: $('#res<%= tblRes & ti("id") %>').val() }, function (data) { eval(data) });
                            return false;
                        });
                    </script>
                    <%
                    else
                        response.write("<hr class='short alt'> "& Resultado)
                    end if
                end if
                %>
                    </div>
