...
function centralSMS(checado, DataAgendamento, HoraAgendamento, AgendamentoID)
	...
	if checado="S" then
		...
		if cdate(DataAgendamento&" "&HoraAgendamento)>=cdate( dateadd( "h", 1, now() ) ) then
			...
			Mensagem = replace(Mensagem, "[HoraAgendamento]", cdate( hour(age("Hora"))&":"&minute(age("Hora")) ) )
...
function limpa(limtabela, limcoluna, limid)
	Set objWinHttp = Server.CreateObject("WinHttp.WinHttpRequest.5.1")
	objWinHttp.Open "GET", "http://192.168.193.43/weegow/feegowclinic/RTFtoHTML.php?banco="&session("banco")&"&tabela="&limtabela&"&coluna="&limcoluna&"&id="&limid
	objWinHttp.Send
