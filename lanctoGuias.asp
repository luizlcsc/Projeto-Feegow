...
if not g.eof then
    set ContasSQL = db.execute("SELECT conta.id, itensinvoice.Descricao "&_
                               ...
                               " WHERE conta.AccountID="&g("ConvenioID")&" AND conta.AssociationAccountID=6 AND conta.CD='C' AND itensinvoice.Tipo='O' AND itensinvoice.Descricao LIKE 'lote%' AND conta.sysDate > DATE_SUB(CURDATE(), INTERVAL 60 DAY)")
end if
if g("Total") > 0 then
%>
<div class="btn-group">
