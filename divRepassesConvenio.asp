...
while not dom.eof
	esteponto = 0
	queima = 0
	if instr(dom("Formas"), "|C|")>0 or instr(dom("Formas"), "|"&ref("ConvenioID")&"|")>0 then
...
if ref("E")="" then
	...
	if request.QueryString("Change")="Profissional" then
		...
		set fun = db.execute("select * from rateiofuncoes where DominioID="&dominiomaior&" and Valor>0")
		while not fun.eof
...
if strRep="db" then
	set repasses = db.execute("select * from rateiorateios where ItemGuiaID="&id)
