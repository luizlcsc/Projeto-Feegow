          ...
          if (instr(session("Unidades"), "|0|")>0 or instr(session("Unidades"), "|"&pLoc("UnidadeID")&"|")>0) then
             ...
             <table class="table table-striped table-condensed table-hover" width="100%">
                 ...
                 <%
					...
					%>
                    <tr class="hidden l<%=LocalID%>_0" id="0000" data-id="0000"></tr>
					<%
                    while not Horarios.EOF
						...
						if UltimoProfissional=ProfissionalID then
							HoraDeSQL = HoraDe
						end if
                        ...
                        while Hora<=HoraA
                            ...
                            %>
                            <tr class="l vazio" data-hora="<%=formatdatetime(Hora, 4)%>" data-prof="<%=ProfissionalID%>" id="<%=HoraID%>">
                                <td width="1%"></td>
                            ...
                            %>
                            <tr class="l<%=LocalID%> vazio" data-hora="<%=formatdatetime(Hora, 4)%>" data-prof="<%=ProfissionalID%>" data-id="<%=HoraID%>" id="<%=LocalID&"_"&HoraID%>">
                                <td width="1%"></td>
                                <td width="1%"><button type="button" class="btn btn-xs btn-info"><%= formatdatetime(Hora,4) %></button></td>
                                <td colspan="4">
                        ...
                        wend
						'response.Write("select agendamentos do Prof: "&ProfissionalID&" no Local: "&LocalID&"<br>")
%>
						                    <tr class="hidden l<%=LocalID%>_0" id="2359" data-id="2359"></tr>
                ...
                "where a.LocalID="&LocalID&" and a.Data="&mydatenull(Data)&" and ProfissionalID="&ProfissionalID&" and Hora>="&mytime(HoraDeSQL)&" order by Hora")
				%>//alert(<%=mytime(HoraDe)%>);<%
                while not comps.EOF
                ...
                $( ".l<%=LocalID%>" ).each(function(){
                    if( $(this).attr("data-id")=='<%=HoraComp%>' && $(this).attr("data-prof")=='<%=ProfissionalID%>' )
                    {
                        $(this).replaceWith('<%= conteudo %>');
                        return false;
                    }
                    else if ( $(this).attr("data-id")>'<%=HoraComp%>' && $(this).attr("data-prof")=='<%=ProfissionalID%>' )
                    {
...
<%						
						strProfissionais = strProfissionais & ", " & ProfissionalID
						UltimoProfissional = ProfissionalID
                    Horarios.movenext
                    wend
                    Horarios.close
                    set Horarios=nothing
					if strProfissionais="" then
						'response.Write("select agendamentos no Local: "&LocalID)
						strWhereProf = ""
					else
						'response.Write("select agendamentos no Local: "&LocalID&" do Prof: NOT IN : "&strProfissionais&"<br>")
						strWhereProf = " and ProfissionalID NOT IN("& right( strProfissionais, len(strProfissionais)-2 ) &")"
					end if
                  %>
                  </tbody>
                <script>
                ...
                "left join pacientes p on p.id=a.PacienteID "&_ 
                "left join profissionais pro on pro.id=a.ProfissionalID "&_ 
                "where a.LocalID="&LocalID&" and a.Data="&mydatenull(Data)&" "&strWhereProf&" order by Hora")
                while not comps.EOF
                    HoraComp = HoraToID(comps("Hora"))
					compsHora = comps("Hora")
					if not isnull(compsHora) then
