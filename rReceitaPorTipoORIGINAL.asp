            ...
            while not dataNF.eof
                ...
                while not gi.eof
                    ...
                    elseif gi("TipoGuia")="guiasadt" then
                        ...
                        set proc = db.execute("select gps.ProfissionalID, gps.id, pac.NomePaciente, conv.NomeConvenio, proc.NomeProcedimento, tiproc.TipoProcedimento, gps.Data, prof.NomeProfissional, gps.ValorTotal, ((gs.TotalGeral-gs.Procedimentos)/(select count(id) from tissprocedimentossadt where GuiaID=gs.id)) ValorDespesas from tissguiasadt gs LEFT JOIN tissprocedimentossadt gps ON gs.id=gps.GuiaID LEFT JOIN procedimentos proc ON proc.id=gps.ProcedimentoID LEFT JOIN tiposprocedimentos tiproc ON tiproc.id=proc.TipoProcedimentoID LEFT JOIN profissionais prof ON prof.id=gps.ProfissionalID LEFT JOIN pacientes pac ON pac.id=gs.PacienteID LEFT JOIN convenios conv ON conv.id=gs.ConvenioID WHERE ifnull(proc.TipoProcedimentoID, 0)="& ptip("id") &" and gs.id="& gi("GuiaID")&" ORDER BY prof.NomeProfissional")
                        if not proc.eof then
...
while not dataNF.eof
    ...
    while not gi.eof
        set proc = db.execute("select pac.NomePaciente, conv.NomeConvenio, (gs.ValorPago-gs.Procedimentos) ValorDespesas from tissguiasadt gs LEFT JOIN pacientes pac ON pac.id=gs.PacienteID LEFT JOIN convenios conv ON conv.id=gs.ConvenioID WHERE gs.id="& gi("GuiaID")&"")
        if not proc.eof then
            NomePaciente = proc("NomePaciente")
            NomeConvenio = proc("NomeConvenio")
            ValorDespesas = proc("ValorDespesas")
