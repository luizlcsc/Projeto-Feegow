...
if Tipo="Profissionais" then
	if ItemID<>"0" then
		set reg = db.execute("select * from tissprofissionaishonorarios where id="&ItemID)
		if not reg.eof then
...
elseif Tipo="Procedimentos" then
	...
	if ItemID<>"0" then
		set reg = db.execute("select * from tissprocedimentoshonorarios where id="&ItemID)
		if not reg.eof then
			ProfissionalID = reg("ProfissionalID")
			Data = reg("Data")
...
elseif Tipo="Despesas" then
	...
	%>
  	<div class="row">
            ...
            <%= quickField("simpleSelect", "TabelaProdutoID", "Tabela", 2, TabelaProdutoID, "select *, concat(id, ' - ', descricao) desccomp from tisstabelas order by id", "desccomp", " empty='' required='required' onchange=""tissCompletaDados(6, this.value+'_'+$('#ProdutoID').val());"" no-select2 ") %>
...
<div class="modal-footer">
    ...
    <button class="btn btn-sm btn-default" type="button" onclick="itemHonorarios('<%=Tipo %>', 0, <%=ItemID %>, 'Cancela');">
