...
%>
	<div class="col-xs-12 clearfix form-actions">
        ...
        <div class="row">
			<%=quickField("empresaMulti", "CompanyUnitID", "Unidades", 6, Unidades, "", "", "")%>
        </div>
...
''				
		...
		if (screenType="Statement" or CD=ref("AccountID")) and getMovement("Date")>=session("DateFrom") and getMovement("Date")<=session("DateTo") then
			...
			if Omite="" then
				...
				GranTotalPago = GranTotalPago+TotalPago
			%>
			<tr>
	...
	set getMovement = nothing
			...
			if ScreenType<>"Statement" then
				...
				<tr>
                    ...
                    <th class="text-right"><%=formatnumber(GranTotalPago, 2)%></th>
                </tr>
                </td>
				<%
			end if
