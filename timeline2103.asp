                ...
                <div class="col-md-3">
                    <div class="btn-group btn-block">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            <%
			            set forms = db.execute("select * from buiforms where sysActive=1 and "& sqlForm &" order by Nome")
			            while not forms.eof
...
%>
    ...
    if instr(Tipo, "|Pedido|")>0 then
        ...
        sqlPedido = sqlPedido & " union all (select id, '', sysUser, 'PedidosSADT', 'Pedido SP/SADT', 'hospital-o', 'system', `Data`, IndicacaoClinica from pedidossadt WHERE PacienteID="&PacienteID&") "
    end if
            ...
            <div class="timeline-item">
                ...
                <div class="panel">
                    <div class="panel-heading">
                        ...
                        <% if Tipo="|L|" then %>
                                <%
                                set vcaSigla = db.execute("select id, NomeProcedimento, sigla from procedimentos where sigla not like '' and TipoProcedimentoID=3 ORDER BY NomeProcedimento")
                                if not vcaSigla.eof and (session("banco")="clinic105" or session("banco")="clinic100000") then
                                    set vcaWL = db.execute("select id from worklist where FormID="&ti("id"))
                                    if vcaWL.eof then
                                    %>
                                <span class="dropdown">
                                  <button class="btn btn-primary dropdown-toggle" type="button" data-toggle="dropdown"><i class="fa fa-laptop"></i> Enviar Worklist
                                  <span class="caret"></span></button>
                                  <ul class="dropdown-menu">
                                      <%while not vcaSigla.eof %>
                                    <li><a href="javascript:sendWorklist(<%=vcaSigla("id") %>, <%=ti("id") %>)"><%=vcaSigla("NomeProcedimento") %></a></li>
                                      <%vcaSigla.movenext
                                          wend
                                          vcaSigla.close
                                          set vcaSigla=nothing%>
                                  </ul>
                                </span>
                            <%
                                  end if
...
'                response.Write( Rotulo & Valor  &"<br>{{"& ti("Tipo") &"}}" )
                select case ti("Tipo")
                    ...
                    response.Write("</small>")
                    case "Diagnostico", "Pedido", "Prescricao", "Atestado"
