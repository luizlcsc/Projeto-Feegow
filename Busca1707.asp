...
elseif len(trim(q))<2 then
	...
	q = replace(replace(q, "'", ""), " ", "%")
	set contaPac = db.execute("select count(*) as Pacientes from pacientes where NomePaciente like '%"&q&"%' or CPF like '%"&q&"%' or Tel1 like '%"&q&"%' or Tel2 like '%"&q&"%' or Cel1 like '%"&q&"%' or Cel2 like '%"&q&"%' or id = '"&q&"'")
	set contaPro = db.execute("select count(*) as Profissionais from profissionais where NomeProfissional like '%"&q&"%'")
	set contaFun = db.execute("select count(*) as Funcionarios from funcionarios where NomeFuncionario like '%"&q&"%'")
	set contaFor = db.execute("select count(*) as Fornecedores from fornecedores where NomeFornecedor like '%"&q&"%'")
	set contaProc = db.execute("select count(*) as Procedimentos from procedimentos where NomeProcedimento like '%"&q&"%'")
	set contaConv = db.execute("select count(*) as Convenios from convenios where NomeConvenio like '%"&q&"%'")
	set contaProd = db.execute("select count(*) as Produtos from produtos where NomeProduto like '%"&q&"%'")
