...
set a = origem.execute("select a.*, p.id PacienteID, p.ConvenioID1, a.PACIENTE, a.TELEFONE from marcacoes a LEFT JOIN clinic1615.pacientes p on p.NomePaciente=a.PACIENTE where not isnull(a.PACIENTE)")
while not a.eof
	TipoCompromissoID = 32
	StaID = 1
	notas = a("OBS")
	if not isnull(a("PacienteID")) then
		PacienteID = a("PacienteID")
	else
		PacienteID = 0
	end if
	if isnull(a("ConvenioID1")) or a("ConvenioID1")=0 then
		rdValorPlano = "V"
		ValorPlano = 0
	else
		rdValorPlano = "P"
		ValorPlano = a("ConvenioID1")
	end if
	Data = a("DATA")
	Data = left(Data, 4) &"-"& mid(Data, 5, 2) &"-"& right(Data,2)
	Hora = dateadd("n", a("HORA"), "00:00:00")
	...
	"VALUES ("&PacienteID&", "&ProfissionalID&", "&mydateNULL(Data)&", "&mytime(Hora)&", "&TipoCompromissoID&", "&StaID&", "&ValorPlano&", '"&rdValorPlano&"', '"&rep(notas)&"', NULL, 0, 7, 0, "&mytime(a("HORA"))&", '"&rep(a("PACIENTE"))&"', '"&rep(a("TELEFONE"))&"')")
a.movenext
wend
a.close
set a=nothing
